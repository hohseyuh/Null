...
- **Every write is exactly one git commit, touching exactly one file.** `create_note` → "Add `<path>`"; `write_note` → "Update `<path>`"; `delete_note` → "Delete `<path>`". Never batched, never combined with any other change. `internal/vault/write.go` serializes every stage-then-commit sequence behind a mutex so two concurrent tool calls can never land in the same commit.
